...
birth_weight_file = r"D:\06_scientific research\01_train_roi\02_roistas_csv\02_filled\downerwoods_DB_1000_roistats.csv"
...
for i in Y:
    if len(Y) == 366:    #这里的10为你想要固定长度list的长度
       print(Y)  # i.append(0)     #添加list a 中的最后一个元素
        ...
        m = random.uniform(0.33000, 0.34000)
        Y.append(m)
...
plt.axvline(x=295,ls='--',c="blue",label = "EOS")#添加垂直直线;'-', '--', '-.', ':', 'None', ' ', '', 'solid', 'dashed', 'dashdot', 'dotted'
...
plt.title("The value of Gcc in Downerwoods")
plt.legend(loc = "best")
...
plt.savefig(r"D:\06_scientific research\07_result\02_GccScatter\2_Downerwoods.tif")
plt.show()
